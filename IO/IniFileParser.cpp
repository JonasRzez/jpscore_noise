...
bool IniFileParser::ParseVelocityModel_adapt(TiXmlElement* xVelocity, TiXmlElement* xMainNode)
{
    ...
    if (xModelPara->FirstChild("e_noise")){
        if (!xModelPara->FirstChildElement("e_noise")->Attribute("mu"))
             ...
             _config->set_emu(atof(emu.c_str()));
        }
        ...
        if (!xModelPara->FirstChildElement("e_noise")->Attribute("sigma"))
             ...
             _config->set_esigma(atof(esigma.c_str()));
        }
     ...
     _config->SetModel(std::shared_ptr<OperationalModel>(new VelocityModel_adapt(_exit_strategy, _config->GetaPed(),
               _config->GetDPed(), _config->GetaWall(),
               _config->GetDWall(), _config->get_esigma(), _config->get_emu())));
     ...
     for(TiXmlElement* xAgentPara = operativModel->FirstChildElement("agent_parameters"); xAgentPara;
         xAgentPara = xAgentPara->NextSiblingElement("agent_parameters")) {
          ...
          if (std::find(usedAgentParams.begin(), usedAgentParams.end(), para_id) != usedAgentParams.end() ) {
              ...
              if (xAgentPara->FirstChild("enoise")) {
                   double mu = xmltof(xAgentPara->FirstChildElement("enoise")->Attribute("mu"));
                   double sigma = xmltof(xAgentPara->FirstChildElement("enoise")->Attribute("sigma"));
                   _config->set_emu(mu);
                   _config->set_esigma(sigma);
              }
